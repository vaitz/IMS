...
const ReportHours = ({ username }) => {
  ...
  const addTime = (time1, time2) => {
    ...
    if (time1Minutes + time2Minutes > 60) {
      return `${("0" + (time1Hours + time2Hours + 1)).slice(-2)}:${(
        "0" +
        (time1Minutes + time2Minutes - 60)
      ).slice(-2)}`;
    }
    return `${("0" + (time1Hours + time2Hours)).slice(-2)}:${(
      "0" +
  ...
  const onAddHour = (date, start, end) => {
    ...
    const newHours = [
      ...hours,
      {
        ...
        startTime: startTime,
        endTime: endTime,
        totalTime: getHoursDiff(startTime, endTime),
        approved: false,
      },
    ];
    setHours(newHours);
  ...
  return (
    <ContentContainer dir="rtl">
      <Container>
        ...
        <Center>
          <ButtonWrapper
            onClick={() => onAddHour(date, start, end)}
            disabled={start >= end}
            value={"הוסף שעות"}
          />
        </Center>
