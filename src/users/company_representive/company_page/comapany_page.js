...
const CompanyPage = () => {
    ...
    const onSubmit = () => {
        createCompanyPage(setError,setPopup,companyName,employees,location,yearEstablish,about);
    }
    ...
    return (
        <Container>
            ...
            <Input type="text" value={yearEstablish} onChange={e => setYearEstablish(e.target.value)}/>
            <Label>אודות</Label>
            <textarea rows="4" cols="50" value={about} onChange={(e) => setAbout(e.target.value)}/>
            {error && <><small style={{ color: 'red' }}>{error}</small><br /></>}<br />
            <Button onClick={() => onSubmit()} disabled={!(companyName && employees && location && yearEstablish && about)}>שמור</Button>
        </Container>
    )
}
